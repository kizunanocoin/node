#include <kizunano/boost/process/child.hpp>
#include <kizunano/crypto_lib/random_pool.hpp>
#include <kizunano/lib/errors.hpp>
#include <kizunano/lib/rpcconfig.hpp>
#include <kizunano/lib/threading.hpp>
#include <kizunano/lib/tomlconfig.hpp>
#include <kizunano/lib/utility.hpp>
#include <kizunano/lib/walletconfig.hpp>
#include <kizunano/kizunano_wallet/icon.hpp>
#include <kizunano/node/cli.hpp>
#include <kizunano/node/daemonconfig.hpp>
#include <kizunano/node/ipc/ipc_server.hpp>
#include <kizunano/node/json_handler.hpp>
#include <kizunano/node/node_rpc_config.hpp>
#include <kizunano/qt/qt.hpp>
#include <kizunano/rpc/rpc.hpp>
#include <kizunano/secure/working.hpp>
...
{
	QMessageBox message (QMessageBox::NoIcon, "Help", "see <a href=\"https://docs.kizunanocoin.com/commands/command-line-interface/#launch-options\">launch options</a> ");
	...
	if (!error)
	{
		...
		if (!node->init_error ())
		{
			...
			if (config.pow_server.enable)
			{
				...
				{
					splash->hide ();
					show_error (std::string ("kizunano_pow_server is configured to start as a child process, however the file cannot be found at: ") + config.pow_server.pow_server_path);
					std::exit (1);
				}
...
#else
				splash->hide ();
				show_error ("kizunano_pow_server is configured to start as a child process, but this is not supported on this system. Disable startup and start the server manually.");
				std::exit (1);
#endif
			}
