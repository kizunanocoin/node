...
TEST (ledger, bootstrap_rep_weight)
{
	...
	nano::work_pool pool (std::numeric_limits<unsigned>::max ());
	{
		...
		store->initialize (transaction, genesis, ledger.cache);
		ASSERT_FALSE (store->account_get (transaction, nano::test_genesis_key.pub, info1));
		nano::send_block send (info1.head, key2.pub, nano::total_supply - 50, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *pool.generate (info1.head));
		ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send).code);
	}
	...
	}
	{
		auto transaction (store->tx_begin_write ());
		ASSERT_FALSE (store->account_get (transaction, nano::test_genesis_key.pub, info1));
		nano::send_block send (info1.head, key2.pub, nano::total_supply - 100, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *pool.generate (info1.head));
		ASSERT_EQ (nano::process_result::progress, ledger.process (transaction, send).code);
	}
