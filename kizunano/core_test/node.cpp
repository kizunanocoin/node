#include <kizunano/core_test/testutil.hpp>
#include <kizunano/lib/jsonconfig.hpp>
#include <kizunano/node/election.hpp>
#include <kizunano/node/testing.hpp>
#include <kizunano/node/transport/udp.hpp>
...
TEST (node, balance)
{
	...
	ASSERT_EQ (nano::total_supply, system.nodes[0]->ledger.account_balance (transaction, nano::test_genesis_key.pub));
}
...
TEST (node, send_self)
{
	...
	ASSERT_EQ (nano::total_supply - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (nano::test_genesis_key.pub));
}
...
TEST (node, send_out_of_order)
{
	...
	nano::send_block send1 (genesis.hash (), key2.pub, nano::total_supply - node1.config.receive_minimum.number (), nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (genesis.hash ()));
	nano::send_block send2 (send1.hash (), key2.pub, nano::total_supply - node1.config.receive_minimum.number () * 2, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (send1.hash ()));
	nano::send_block send3 (send2.hash (), key2.pub, nano::total_supply - node1.config.receive_minimum.number () * 3, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (send2.hash ()));
...
TEST (node_config, v15_v16_upgrade)
{
	...
	test_upgrade ("rai.raiblocks.net", "peering.kizunanocoin.com");
	test_upgrade ("rai-beta.raiblocks.net", "peering-beta.kizunanocoin.com");
}
...
TEST (node_config, v16_values)
{
	...
	tree.put ("vote_minimum", (nano::total_supply - 100).convert_to<std::string> ());
	upgraded = false;
	config.deserialize_json (upgraded, tree);
	ASSERT_FALSE (upgraded);
	ASSERT_TRUE (config.allow_local_peers);
	ASSERT_EQ (config.signature_checker_threads, 4);
	ASSERT_EQ (config.vote_minimum.number (), nano::total_supply - 100);
}
...
TEST (node, rep_self_vote)
{
	nano::system system;
	nano::node_config node_config (nano::get_available_port (), system.logging);
	node_config.online_weight_minimum = nano::total_supply;
	node_config.frontiers_confirmation = nano::frontiers_confirmation_mode::disabled;
	auto node0 = system.add_node (node_config);
	nano::keypair rep_big;
	nano::send_block fund_big (node0->ledger.latest (node0->store.tx_begin_read (), nano::test_genesis_key.pub), rep_big.pub, nano::total_supply, nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0);
	nano::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
	node0->work_generate_blocking (fund_big);
	node0->work_generate_blocking (open_big);
	...
	system.wallet (0)->insert_adhoc (rep_big.prv);
	system.wallet (0)->insert_adhoc (nano::test_genesis_key.prv);
	ASSERT_EQ (system.wallet (0)->wallets.reps ().voting, 1);
	auto block0 (std::make_shared<nano::send_block> (node0->latest (nano::test_genesis_key.pub), rep_big.pub, nano::uint128_t ("0x40000000000000"), nano::test_genesis_key.prv, nano::test_genesis_key.pub, 0));
...
TEST (node, send_callback)
{
	...
	ASSERT_EQ (nano::total_supply - node0.config.receive_minimum.number (), node0.balance (nano::test_genesis_key.pub));
}
...
TEST (node, confirm_quorum)
{
	...
	{
		auto transaction (node1.store.tx_begin_write ());
		ASSERT_NE (nano::process_result::progress, node1.ledger.process (transaction, *send1).code);
	}
...
TEST (node, vote_republish)
{
	...
	system.wallet (1)->insert_adhoc (key2.prv);
	nano::genesis genesis;
	auto send1 (std::make_shared<nano::send_block> (genesis.hash (), key2.pub, nano::total_supply - node1.config.receive_minimum.number (), nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto send2 (std::make_shared<nano::send_block> (genesis.hash (), key2.pub, nano::total_supply - node1.config.receive_minimum.number () * 2, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	node1.process_active (send1);
	system.deadline_set (5s);
...
TEST (node, aggressive_flooding)
{
	...
	ASSERT_EQ (2 + 2 * nodes_wallets.size () + 2, node1.ledger.cache.block_count);
}
