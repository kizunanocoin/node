...
{
	nano::amount amount;
	ASSERT_FALSE (amount.decode_dec ("20000000000000000", nano::raw_ratio));
	ASSERT_EQ (nano::total_supply, amount.number ());
	ASSERT_TRUE (amount.decode_dec ("20000000000000001", nano::raw_ratio));
	ASSERT_TRUE (amount.decode_dec ("20000000000000000.1", nano::raw_ratio));
...
{
	nano::account key;
	ASSERT_FALSE (key.decode_account ("kiz_1111111111111111111111111111111111111111111111111111hifc8npp"));
	ASSERT_FALSE (key.decode_account ("kizn_1111111111111111111111111111111111111111111111111111hifc8npp"));
}
