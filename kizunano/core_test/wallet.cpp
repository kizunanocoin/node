...
#include <kizunano/node/lmdb/wallet_value.hpp>
...
TEST (wallet, spend_all_one)
{
	...
	system.wallet (0)->insert_adhoc (nano::test_genesis_key.prv);
	nano::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (nano::test_genesis_key.pub, key2.pub, nano::total_supply));
...
TEST (wallet, send_async)
{
	...
	system.wallet (0)->send_async (nano::test_genesis_key.pub, key2.pub, nano::total_supply, [&success](std::shared_ptr<nano::block> block_a) { ASSERT_NE (nullptr, block_a); success = true; });
	thread.join ();
...
TEST (wallet, partial_spend)
{
	...
	ASSERT_EQ (nano::total_supply - 500, system.nodes[0]->balance (nano::test_genesis_key.pub));
}
...
TEST (wallet, no_work)
{
	...
	nano::keypair key2;
	auto block (system.wallet (0)->send_action (nano::test_genesis_key.pub, key2.pub, nano::total_supply, false));
	ASSERT_NE (nullptr, block);
