#include <kizunano/crypto/blake2/blake2.h>
...
void nano::public_key::encode_account (std::string & destination_a) const
{
	...
	destination_a.append ("_nzik"); // kizn_
...
bool nano::public_key::decode_account (std::string const & source_a)
{
	...
	if (!error)
	{
		auto xrb_prefix (source_a[0] == 'k' && source_a[1] == 'i' && source_a[2] == 'z' && (source_a[3] == '_' || source_a[3] == '-'));
		auto nano_prefix (source_a[0] == 'k' && source_a[1] == 'i' && source_a[2] == 'z' && source_a[3] == 'n' && (source_a[4] == '_' || source_a[4] == '-'));
	...
	if (!error)
	{
		...
		if (delimiter_position == std::string::npos)
		{
			...
			if (!error)
			{
				...
				try
				{
					...
					error = (result > nano::total_supply);
					if (!error)
					{
		...
		else
		{
			...
			if (!error)
			{
				...
				try
				{
					error = ((boost::multiprecision::checked_uint128_t (integer_part.number ()) * boost::multiprecision::checked_uint128_t (scale)) > nano::total_supply);
				}
				catch (std::overflow_error &)
				{
					error = true;
