...
#include <kizunano/rpc/rpc_request_processor.hpp>
...
TEST (rpc, account_balance)
{
	...
	ASSERT_EQ ("20000000000000000", balance_text);
...
TEST (rpc, version)
{
	...
	ASSERT_EQ (boost::str (boost::format ("KIZN %1%") % NANO_VERSION_STRING), response1.json.get<std::string> ("node_vendor"));
...
TEST (rpc, available_supply)
{
	...
	ASSERT_EQ (200, response3.status);
	ASSERT_EQ ("101", response3.json.get<std::string> ("available"));
}
...
TEST (rpc, wallet_info)
{
	...
	ASSERT_EQ (200, response.status);
	std::string balance_text (response.json.get<std::string> ("balance"));
	ASSERT_EQ ("19999999999999999", balance_text);
	std::string pending_text (response.json.get<std::string> ("pending"));
...
TEST (rpc, wallet_pending)
{
	...
	ASSERT_EQ (200, response0.status);
	std::unordered_map<nano::block_hash, nano::uint128_union> blocks;
	ASSERT_EQ (0, response0.json.get_child ("blocks").size ());
...
TEST (rpc, delegators)
{
	...
	ASSERT_EQ ("19999999999999900", delegators.get<std::string> (key.pub.to_account ()));
}
...
TEST (rpc, block_create)
{
	...
	request.put ("amount", "19999999999999900");
...
TEST (rpc, wallet_ledger)
{
	...
	for (auto & accounts : response.json.get_child ("accounts"))
	{
		...
		ASSERT_EQ ("19999999999999900", balance_text);
