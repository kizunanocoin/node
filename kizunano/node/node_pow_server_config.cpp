...
#include <kizunano/node/node_pow_server_config.hpp>
...
nano::error nano::node_pow_server_config::serialize_toml (nano::tomlconfig & toml) const
{
	...
	toml.put ("kizunano_pow_server_path", pow_server_path, "Value is currently not in use. Path to the nano_pow_server executable.\ntype:string,path");
...
nano::error nano::node_pow_server_config::deserialize_toml (nano::tomlconfig & toml)
{
	...
	toml.get_optional<std::string> ("kizunano_pow_server_path", pow_server_path);
