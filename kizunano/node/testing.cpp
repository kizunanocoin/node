...
void nano::system::generate_receive (nano::node & node_a)
{
	...
	{
		auto receive_error (wallet (0)->receive_sync (send_block, nano::genesis_account, nano::total_supply));
		(void)receive_error;
	}
}
...
nano::uint128_t nano::system::get_random_amount (nano::transaction const & transaction_a, nano::node & node_a, nano::account const & account_a)
{
	...
	return (((nano::uint256_t{ random_amount.number () } * balance) / nano::uint256_t{ nano::total_supply }).convert_to<nano::uint128_t> ());
}
