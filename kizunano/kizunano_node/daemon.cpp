...
void nano_daemon::daemon::run (boost::filesystem::path const & data_path, nano::node_flags const & flags)
{
	...
	if (!error)
	{
		...
		try
		{
			...
			auto initialization_text = "Starting up KIZUNANO COIN node...";
			...
			if (!node->init_error ())
			{
				...
				/*if (config.pow_server.enable)
				{
					...
					{
						std::cerr << std::string ("kizunano_pow_server is configured to start as a child process, however the file cannot be found at: ") + config.pow_server.pow_server_path << std::endl;
						std::exit (1);
					}
...
#else
					std::cerr << "kizunano_pow_server is configured to start as a child process, but this is not supported on this system. Disable startup and start the server manually." << std::endl;
					std::exit (1);
#endif
				}*/
